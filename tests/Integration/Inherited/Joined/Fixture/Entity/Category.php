...
#[ORM\Entity]
#[ORM\Table(name: 'categories_inherited_joined')]
class Category
{
    #[ORM\Id]
    #[ORM\Column]
    ...
    #[Formula('(SELECT COALESCE(SUM(p.total), 0) FROM products_inherited_joined_formula p JOIN category_products_inherited_joined cp ON cp.product_id = p.id WHERE cp.category_id = {this}.id)')]
    public float $categoryRevenue = 0.0;

    // DQL formula — uses Product.orderCount formula field via DQL
    #[Formula('SELECT COALESCE(SUM(p.orderCount), 0) FROM ' . FormulaJoinedProduct::class . ' p JOIN p.categories cat WHERE cat = {this}')]
    public int $amountOrders = 0;

    public function __construct(
        #[ORM\ManyToMany(targetEntity: FormulaJoinedProduct::class, inversedBy: 'categories')]
        #[ORM\JoinTable(
            name: 'category_products_inherited_joined',
