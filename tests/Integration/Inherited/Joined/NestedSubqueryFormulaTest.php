...
final class NestedSubqueryFormulaTest extends JoinedInheritedOrmTestCase
{
    public function testDqlFourLevelNestedSubquery(): void
    {
        ...
        $ratings = $this->em->createQuery(
            // Level 1: select Ratings joined with their Products (as FormulaJoinedProduct), filter by formula field stars
            'SELECT rt FROM ' . Rating::class . ' rt ' .
            'JOIN ' . FormulaJoinedProduct::class . ' p WITH p.id = rt.product ' .
            ...
            'AND p.id IN (' .
            '    SELECT p2.id FROM ' . FormulaJoinedProduct::class . ' p2 ' .
            '    GROUP BY p2.id ' .
        ...
        self::assertInstanceOf(FormulaJoinedProduct::class, $ratings[0]->product);
        self::assertSame('Product 1', $ratings[0]->product->name);
        self::assertSame(90.0, $ratings[0]->product->totalRevenue);

        self::assertInstanceOf(FormulaJoinedProduct::class, $ratings[1]->product);
