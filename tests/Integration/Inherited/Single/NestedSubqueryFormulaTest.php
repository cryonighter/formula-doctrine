...
final class NestedSubqueryFormulaTest extends SingleInheritedOrmTestCase
{
    public function testDqlFourLevelNestedSubquery(): void
    {
        ...
        $ratings = $this->em->createQuery(
            // Level 1: select Ratings joined with their Products (as FormulaSingleProduct), filter by formula field stars
            'SELECT rt FROM ' . Rating::class . ' rt ' .
            'JOIN ' . FormulaSingleProduct::class . ' p WITH p.id = rt.product ' .
            ...
            'AND p.id IN (' .
            '    SELECT p2.id FROM ' . FormulaSingleProduct::class . ' p2 ' .
            '    GROUP BY p2.id ' .
        ...
        self::assertInstanceOf(FormulaSingleProduct::class, $ratings[0]->product);
        self::assertSame('Product 1', $ratings[0]->product->name);
        self::assertSame(90.0, $ratings[0]->product->totalRevenue);

        self::assertInstanceOf(FormulaSingleProduct::class, $ratings[1]->product);
