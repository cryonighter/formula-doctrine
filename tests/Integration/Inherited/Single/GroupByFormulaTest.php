...
final class GroupByFormulaTest extends SingleInheritedOrmTestCase
{
    public function testDqlGroupByFormulaField(): void
    {
        ...
        $result = $this->em->createQuery(
            'SELECT p.orderCount, COUNT(p.id) as productCount FROM ' . FormulaSingleProduct::class . ' p GROUP BY p.orderCount'
        )->getResult();
    ...
    public function testDqlGroupByMultipleFields(): void
    {
        ...
        $result = $this->em->createQuery(
            ...
            'FROM ' . FormulaSingleProduct::class . ' p GROUP BY p.name, p.orderCount ' .
            'ORDER BY p.name'
        )->getResult();
